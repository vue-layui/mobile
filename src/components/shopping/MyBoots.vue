<template>
    <div class="bigBox">
      <ul>
        <li  :key="index" v-for="(p , index ) in list" @click="local(p) ; goproduct()">
          <p><img :src="p.imgs1" alt=""></p>
          <p>{{p.title1}}</p>
          <p>{{p.title2}}</p>
          <p>
            <span>￥<i>{{p.pic1}}</i></span>
            <span>￥<i>{{p.pic2}}</i></span>
            <img :src="p.imgs2" alt="">
          </p>
          <p class="px">
            <span>{{p.pref1}}</span>
            <span>{{p.pref2}}</span>
            <span>
              <img :src="p.imgs3" alt=""><i>{{p.pic3}}</i>
            </span>
          </p>
          <p class="ps">
            <span>{{p.prop}}</span>
...
<script>
    export default {
      ...
      methods:{
        local(p){
            console.log(p);
            let objs = JSON.stringify(p)
            window.localStorage.setItem("list", objs);
            // console.log
        },
          goproduct(){
